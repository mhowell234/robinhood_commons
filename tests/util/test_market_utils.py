...
from robinhood_commons.util.market_utils import in_extended_hours_market_time_window, in_market_time_window, \
    ...
    in_pre_market_time_window, is_extended_hours_market_open, is_market_open, time_til_regular_close, CLOSE_TIME, OPEN_TIME, \
...
def test_time_til_extended_hours_close_market_not_open_max_time():
    before_after_hours_market_open, _ = generate_date_times(a_date=MARKET_OPEN_DATE, a_time=PRE_MARKET_OPEN_TIME)

    assert timedelta.max == time_til_regular_close(a_date=before_after_hours_market_open)
